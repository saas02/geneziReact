$(document).ready(function() {
    ...
    $('#add-categori form').submit(function(e) {
         ...
         $.ajax({
            ...
            success: function (data) {
                if(data.error){                   
                    jQuery('#res-form-add-categori').html(data.error);
                }else{
                    $("#res-form-add-categori").html(data.message+'<br><img src="assets/img/loading.gif" class="center-all-contens">');
    ...
    $('#add-provee form').submit(function(e) {
         ...
         $.ajax({
            ...
            success: function (data) {                
                if(data.error){                   
                    jQuery('#res-form-add-prove').html(data.error);
                }else{
                    $("#res-form-add-prove").html(data.message+'<br><img src="assets/img/loading.gif" class="center-all-contens">');
                    window.location = data.redirect;                        
                }
            }
        });
	...
	var myId = $(this).val();
        $('#update-category form#'+myId).submit(function(e) {
             ...
             $.ajax({
                ...
                success: function (data) {
                    if(data.error){                   
                        $("div#"+myId).html(data.error);
                    }else{
                        $("div#"+myId).html(data.message);
                    }                    
                }
            });
            return false;
    ...
    /*Actualizar Archivos producto con ajax*/
    $('.button-files-UPR').click(function() {
	var myId = $(this).val();        
        $('#update-product form#'+myId).append("<input name='prod-files-update' value='1'>");
        $('#update-product form#'+myId).submit(function(e) {});
    });
    
    /*Actualizar Archivos producto con ajax*/
    $('.button-INA').click(function() {
	var myId = $(this).val();        
        $('#update-product form#'+myId).append("<input name='prod-files-inactivar' value='1'>");        
        $('#update-product form#'+myId).submit(function(e) {
             e.preventDefault();
             var informacion=$('#update-product form#'+myId).serialize();
             var metodo=$('#update-product form#'+myId).attr('method');
    ...
    $('.delete-img').click(function() {
        var opcion = confirm("¿Desea Eliminar la imagen?");
        if (opcion == true) {
            var dataimg = $(this).remove();
	}
        var imagen = '';
        $(".delete-img").each(function() {
            imagen += $(this).data('imagen')+";";
        });
        $('#img-names').val(imagen);
    });    
    ...
    $('#add-admin form').submit(function(e) {
         ...
         $.ajax({
            ...
            success: function (data) {                
                if(data.error){                   
                    jQuery('#res-form-add-admin').html(data.error);
                }else{
                    $("#res-form-add-admin").html(data.message+'<br><img src="assets/img/loading.gif" class="center-all-contens">');
                    window.location = data.redirect;                        
                }
            }
        });
    ...
    /*Envio del formulario con Ajax para añadir contnidos*/
    $('#_submit').click(function(e) {        
        if($("#contentsAdmin").valid()){
            e.preventDefault();
            var informacion=$('#contentsAdmin').serialize();
            var metodo=$('#contentsAdmin').attr('method');
            var peticion=$('#contentsAdmin').attr('action');
            $.ajax({
               type: metodo,
               url: peticion,
               data:informacion,
               beforeSend: function(){
                   $("#res-form-contenido").html('Creando Contenido...<br><img src="assets/img/enviando.gif" class="center-all-contens">');
               },
               error: function() {
                   $("#res-form-contenido").html("Ha ocurrido un error en el sistema");
               },
               success: function (data) {
                   if(data.error){
                       $("#res-form-contenido").html(data.error);
                   }else{
                       $("#res-form-contenido").html(data.message); 
                       window.location = data.redirect;
                   }                
               }
           });
    ...
    if($("#contentsAdmin").length > 0){
        $("#contentsAdmin").validate({           
            cont_titulo: {
                required: true                            
            },
            cont_descipcion: {
